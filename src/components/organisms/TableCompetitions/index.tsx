import React, { useEffect, useState } from 'react';
import Title from '../../atoms/Title';
import './style.css'
import Image from '../../atoms/Image';

import SideButton from '../../atoms/SideButton';


export default function TableCompetitions() {

    const [table, setTable] = useState(true);
    const [ranking, setRanking] = useState(peleiaRxMasc2019);
    const[competitionName, setCompetitionName] = useState('Peleia Games');
    

    const pageProps = {
        margin: 0,
        padding: 0,
        display: 'flex',
        justifyContent: 'center',

    }

    const TableCompetitionProps = {
        width: '100%',
    }

    const boxColumn = {
        display: 'grid',
        alignContent: 'center',
        justifyContent: 'center',
        gap: '5px',
    }

    
    const buttonSection = {
        display: 'flex',
    }

    const notSelectedButton = {
        background: 'none',
    }

    const selectedButton = {
        backgroundColor: '#e6f5ff',
        opacity: '0.9'
    }

    const[supergamesButton, setSupergamesButton] = useState<any>(notSelectedButton);
    const[peleiagamesButton, setPeleiagamesButton] = useState<any>(selectedButton);
    const[sinosgamesButton, setSinosgamesButton] = useState<any>(notSelectedButton);

    return(
        <> 
            <button
                style={{position: 'fixed', top: '80%', right: '10px', zIndex: 100}}
                onClick={() => table ? setTable(false) : setTable(true)}
            >
                <SideButton side={table ? 'right' : 'left'} />
            </button>
            <div>
                <div style={buttonSection}>
                    <button
                        className="buttonTable"
                        style={supergamesButton}
                        onClick={() => {setRanking(supergamesRxMasc2019),
                            setCompetitionName('SuperGames'),
                            setSupergamesButton(selectedButton),
                            setPeleiagamesButton(notSelectedButton),
                            setSinosgamesButton(notSelectedButton)
                        }}
                    >
                        SuperGames
                    </button>
                    <button
                        className="buttonTable"
                        style={sinosgamesButton}
                        onClick={() => {setRanking(sinosRxMasc2019),
                            setCompetitionName('Sinos Games'),
                            setSupergamesButton(notSelectedButton),
                            setPeleiagamesButton(notSelectedButton),
                            setSinosgamesButton(selectedButton)
                        }}
                    >
                        Sinos Games
                    </button>
                    <button
                        className="buttonTable"
                        style={peleiagamesButton}
                        onClick={() => {setRanking(peleiaRxMasc2019),
                            setCompetitionName('Peleia Games'),
                            setSupergamesButton(notSelectedButton),
                            setPeleiagamesButton(selectedButton),
                            setSinosgamesButton(notSelectedButton)
                        }}
                    >
                        Peleia Games
                    </button>    
                </div>
                <div id="TableCompetition" style={pageProps}>
                    {table ? (
                        <table style={TableCompetitionProps}>
                            <tr>
                                <th style={{padding: 5}}><Title title='Rank'/></th>
                                <th style={{padding: 5}}><Title title='Atleta'/></th>
                                <th style={{padding: 5}}><Title title='Box'/></th>
                            </tr>
                            {ranking.ranking.map(list => (
                                <tr>
                                    <td><Title title={list.rank + 'º ('+list.points+')'} /></td>
                                    <td><Title title={list.name} /></td>
                                    <td>
                                        <div style={boxColumn}>
                                            <Image
                                                image={list.boxImage}
                                                width='35px'
                                                title={list.boxName}
                                            />
                                            <Title title={list.boxName} />
                                        </div>
                                    </td>
                                </tr>
                                ))}
                        </table>
                    ) : (
                        <table style={TableCompetitionProps}>
                            <tr>
                                <th>
                                    <div style={{transform: 'rotate(-30deg)'}}>
                                        <Title title='ATLETA'/>
                                    </div>
                                </th>
                                {ranking.workouts.map(list => (
                                    <th>
                                        <div style={{transform: 'rotate(-30deg)'}}>
                                            <Title title={list.workout_name} />
                                        </div>
                                    </th>
                                ))}
                            </tr>
                            {ranking.ranking.map(list => (
                                <tr>
                                    <td>
                                        <div style={boxColumn}>
                                            <Title title={list.rank + 'º ('+ list.points + ')'} />
                                            <Title title={list.name} />
                                        </div>
                                    </td>
                                    {list.workouts.map(wods => (
                                        <td>
                                            <div style={boxColumn}>
                                                <Title title={wods.workoutrank} />
                                                <Title title={'('+wods.workoutresult+')'} />
                                            </div>
                                        </td>
                                    ))}
                                </tr>
                                ))}
                        </table>
                    )}
                </div>
            </div>
        </>
    );
}



const peleiaRxMasc2019 = {
    season: 2019,
    workouts: [
        {
            workout_id: 1,
            workout_name: 'Workout 1'
        },
        {
            workout_id: 2,
            workout_name: 'Workout 2'
        },
        {
            workout_id: 3,
            workout_name: 'Workout 3'
        },
        {
            workout_id: 4,
            workout_name: 'Workout 4'
        },
        {
            workout_id: 5,
            workout_name: 'Workout 5'
        },
    ],
    ranking: [
        {
            rank: '1',
            name: 'Álvaro Teixeira',
            boxImage: 'https://yt3.ggpht.com/a/AATXAJxAF81y3WHDmF3gxfp3grliXJKTgTEvQJu3A-k7=s900-c-k-c0x00ffffff-no-rj',
            boxName: 'CrossFit Caxias do Sul',
            points: '7',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '1',
                    workoutresult: '146 reps',
                },
                {
                    workout_id: 2,
                    workoutrank: '2',
                    workoutresult: '03:34',
                },
                {
                    workout_id: 3,
                    workoutrank: '1',
                    workoutresult: '361 reps',
                },
                {
                    workout_id: 4,
                    workoutrank: '1',
                    workoutresult: '04:13',
                },
                {
                    workout_id: 5,
                    workoutrank: '2',
                    workoutresult: '08:55',
                },
            ],
        },
        {
            rank: '2',
            name: 'Bruno Bassani',
            boxImage: 'https://yt3.ggpht.com/a/AATXAJw6lk_cDCOyhlyyUDX99GxpKVuH2szNEM79Ue-HMA=s900-c-k-c0x00ffffff-no-rj',
            boxName: 'CrossFit BrotherHood',
            points: '16',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '4',
                    workoutresult: '141 reps',
                },
                {
                    workout_id: 2,
                    workoutrank: '3',
                    workoutresult: '04:08',
                },
                {
                    workout_id: 3,
                    workoutrank: '5',
                    workoutresult: '319 reps',
                },
                {
                    workout_id: 4,
                    workoutrank: '3',
                    workoutresult: '04:30',
                },
                {
                    workout_id: 5,
                    workoutrank: '1',
                    workoutresult: '08:25',
                },
            ],
        },
        {
            rank: '3',
            name: 'Igor Silva',
            boxImage: 'https://yt3.ggpht.com/a/AATXAJw6lk_cDCOyhlyyUDX99GxpKVuH2szNEM79Ue-HMA=s900-c-k-c0x00ffffff-no-rj',
            boxName: 'CrossFit BrotherHood',
            points: '18',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '2',
                    workoutresult: '145 reps',
                },
                {
                    workout_id: 2,
                    workoutrank: '5',
                    workoutresult: '04:21',
                },
                {
                    workout_id: 3,
                    workoutrank: '3',
                    workoutresult: '334 reps',
                },
                {
                    workout_id: 4,
                    workoutrank: '4',
                    workoutresult: '05:36',
                },
                {
                    workout_id: 5,
                    workoutrank: '4',
                    workoutresult: '09:58',
                },
            ],
        },
        {
            rank: '4',
            name: 'João Marcelo Santiago',
            boxImage: 'data:image/jpeg;base64,',
            boxName: 'CT Bunker 1194',
            points: '25',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '3',
                    workoutresult: '142 reps',
                },
                {
                    workout_id: 2,
                    workoutrank: '7',
                    workoutresult: '04:28',
                },
                {
                    workout_id: 3,
                    workoutrank: '10',
                    workoutresult: '312 reps',
                },
                {
                    workout_id: 4,
                    workoutrank: '2',
                    workoutresult: '04:28',
                },
                {
                    workout_id: 5,
                    workoutrank: '3',
                    workoutresult: '09:11',
                },
            ],
        },
        {
            rank: '5',
            name: 'Lucas Reis Schuch',
            boxImage: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4QThCkAFnobDwfC9QHcmzdVYZtsfE1vdchg&usqp=CAU',
            boxName: 'CrossFit Equifavela',
            points: '32',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '5',
                    workoutresult: '135 reps',
                },
                {
                    workout_id: 2,
                    workoutrank: '11',
                    workoutresult: '04:54',
                },
                {
                    workout_id: 3,
                    workoutrank: '6',
                    workoutresult: '315 reps',
                },
                {
                    workout_id: 4,
                    workoutrank: '5',
                    workoutresult: '05:53',
                },
                {
                    workout_id: 5,
                    workoutrank: '5',
                    workoutresult: 'CAP+81',
                }
            ]
        },
    ]
}



const supergamesRxMasc2019 = {
    season: 2019,
    workouts: [
        {
            workout_id: 1,
            workout_name: 'WOD 1'
        },
        {
            workout_id: 2,
            workout_name: 'WOD 2'
        },
        {
            workout_id: 3,
            workout_name: 'WOD 3'
        },
        {
            workout_id: 4,
            workout_name: 'WOD 4A'
        },
        {
            workout_id: 5,
            workout_name: 'WOD 4B'
        },
        {
            workout_id: 6,
            workout_name: 'WOD 5'
        },
    ],
    ranking: [
        {
            rank: '1',
            name: 'Vinícius Machado',
            boxImage: 'data:image/png;base64,',
            boxName: 'CrossFit Estância Velha',
            points: '520',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '2',
                    workoutresult: '04:40',
                },
                {
                    workout_id: 2,
                    workoutrank: '13',
                    workoutresult: '05:58',
                },
                {
                    workout_id: 3,
                    workoutrank: '2',
                    workoutresult: '02:37',
                },
                {
                    workout_id: 4,
                    workoutrank: '2',
                    workoutresult: '37',
                },
                {
                    workout_id: 5,
                    workoutrank: '1',
                    workoutresult: '08:51',
                },
                {
                    workout_id: 6,
                    workoutrank: '4',
                    workoutresult: '07:50',
                },
            ],
        },
        {
            rank: '2',
            name: 'Giovanne Costa',
            boxImage: 'data:image/png;base64,',
            boxName: 'Feat Up CrossFit',
            points: '514',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '5',
                    workoutresult: '04:58',
                },
                {
                    workout_id: 2,
                    workoutrank: '1',
                    workoutresult: '03:59',
                },
                {
                    workout_id: 3,
                    workoutrank: '8',
                    workoutresult: '03:08',
                },
                {
                    workout_id: 4,
                    workoutrank: '2',
                    workoutresult: '37',
                },
                {
                    workout_id: 5,
                    workoutrank: '7',
                    workoutresult: '10:26',
                },
                {
                    workout_id: 6,
                    workoutrank: '1',
                    workoutresult: '06:22',
                },
            ],
        },
        {
            rank: '3',
            name: 'Daniel da Rosa',
            boxImage: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ2gP6-QaOklX_S_KSW2OjabcqampRT6K19OA&usqp=CAU',
            boxName: 'VTreino',
            points: '508',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '3',
                    workoutresult: '04:42',
                },
                {
                    workout_id: 2,
                    workoutrank: '2',
                    workoutresult: '04:12',
                },
                {
                    workout_id: 3,
                    workoutrank: '4',
                    workoutresult: '02:44',
                },
                {
                    workout_id: 4,
                    workoutrank: '6',
                    workoutresult: '33',
                },
                {
                    workout_id: 5,
                    workoutrank: '7',
                    workoutresult: '10:26',
                },
                {
                    workout_id: 6,
                    workoutrank: '2',
                    workoutresult: '06:54',
                },
            ],
        },
        {
            rank: '4',
            name: 'Vinícius Dullius',
            boxImage: 'data:image/png;base64,',
            boxName: 'Tork CrossFit',
            points: '496',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '--',
                },
            ],
        },
        {
            rank: '5',
            name: 'Cláudio Rocha',
            boxImage: 'data:image/png;base64,',
            boxName: 'CrossFit Criciúma',
            points: '468',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '--',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '--',
                },
            ]
        },
    ]
}




const sinosRxMasc2019 = {
    season: 2019,
    workouts: [
        {
            workout_id: 1,
            workout_name: 'Workout 1'
        },
        {
            workout_id: 2,
            workout_name: 'Workout 2'
        },
        {
            workout_id: 3,
            workout_name: 'Workout 3'
        },
        {
            workout_id: 4,
            workout_name: 'Workout 4'
        },
        {
            workout_id: 5,
            workout_name: 'Workout 5'
        },
        {
            workout_id: 6,
            workout_name: 'Workout 6'
        },
        {
            workout_id: 7,
            workout_name: 'Workout 7'
        },
    ],
    ranking: [
        {
            rank: '1',
            name: 'Clayton Rodrigues',
            boxImage: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRNZju45JlOfbxyMdD7rf65ur7SGc2exQd96A&usqp=CAU',
            boxName: '303WOD',
            points: '16',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 7,
                    workoutrank: '-',
                    workoutresult: '-',
                },
            ],
        },
        {
            rank: '2',
            name: 'Álvaro Teixeira',
            boxImage: 'https://yt3.ggpht.com/a/AATXAJxAF81y3WHDmF3gxfp3grliXJKTgTEvQJu3A-k7=s900-c-k-c0x00ffffff-no-rj',
            boxName: 'CrossFit Caxias do Sul',
            points: '28',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 7,
                    workoutrank: '-',
                    workoutresult: '-',
                },
            ],
        },
        {
            rank: '3',
            name: 'João Paiva Filho',
            boxImage: 'data:image/png;base64,',
            boxName: 'CrossFit Tribull',
            points: '51',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 7,
                    workoutrank: '-',
                    workoutresult: '-',
                },
            ],
        },
        {
            rank: '4',
            name: 'Luis Schneider',
            boxImage: 'https://dtda4cv2md3ne.cloudfront.net/image/filename/356454/logo_14908298_318797458476593_6716802424421161877_n.jpg',
            boxName: 'CrossFit São Leopoldo',
            points: '53',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 7,
                    workoutrank: '-',
                    workoutresult: '-',
                },
            ],
        },
        {
            rank: '5',
            name: 'Rafael Alves',
            boxImage: 'data:image/png;base64,',
            boxName: 'Estivadores',
            points: '55',
            workouts: [
                {
                    workout_id: 1,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 2,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 3,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 4,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 5,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 6,
                    workoutrank: '-',
                    workoutresult: '-',
                },
                {
                    workout_id: 7,
                    workoutrank: '-',
                    workoutresult: '-',
                },
            ],
        },
    ]
}

